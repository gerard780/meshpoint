...
- **Configuration → Serial's "Meshtastic firmware" card flashes a spare ESP32 board with official Meshtastic firmware straight from the dashboard** — downloads the latest release and writes it with `esptool` (no compiling; Meshtastic firmware is PlatformIO-built upstream), so a board previously running something else entirely (e.g. the DAPNET companion below) can be repurposed into a Meshtastic USB stick without ever leaving the browser. Board choice is a curated pulldown (Heltec V3, TTGO LoRa32 variants); flashing erases the whole chip first and is guarded by a confirm modal, since it's genuinely destructive. **Configuration → MeshCore has the same "MeshCore firmware" card** for repurposing a spare board into a MeshCore USB companion instead — same mechanism, same curated board list, official prebuilt releases only.
