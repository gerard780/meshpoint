...
- **RF Environment companion for boards with no SX1261 at all** — RAK2287 is datasheet-confirmed to have no SX1261, so no config change can ever enable a real hardware spectral scan there. `extra/rfenv_companion` (Heltec V3, own SX1262 + antenna) samples real ambient RSSI over USB serial and feeds the same histogram/noise-floor card instead of the packet-derived fallback; the board's OLED also shows a standalone live band-scan view, independent of any Pi connection.
