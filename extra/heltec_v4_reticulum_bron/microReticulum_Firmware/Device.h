...
bool device_init() {
  if (bt_ready) {
    ...
    return device_init_done; // CBA-patch: vertrouwde zelf-build — signature-eis (fw_signature_validated) verwijderd; die gate'de de radio
  } else {
    device_init_done = true;  // CBA-patch: bt_ready niet vereist voor headless LoRa-node
    return true;
  }
}
#endif
