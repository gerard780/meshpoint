...
// CBA-patch: TCP-client interface naar de VPS (internet-uplink voor standalone node)
#if defined(TCP_VPS)
WiFiClient vps_client;
RNS::Interface tcp_interface(RNS::Type::NONE);
static uint8_t tcp_rx_buf[600];
static size_t  tcp_rx_len = 0;
static bool    tcp_rx_in = false, tcp_rx_esc = false;
static uint32_t vps_last_try = 0;
static bool    vps_was_connected = false;
void update_tcp_vps() {
  if (WiFi.status() != WL_CONNECTED) return;
  if (!vps_client.connected()) {
    if (vps_was_connected) { Serial.println("[VPS] verbinding verbroken"); vps_was_connected = false; }
    if (millis() - vps_last_try < 5000) return;
    vps_last_try = millis();
    Serial.printf("[VPS] verbinden met %s:%d ...\r\n", TCP_VPS_HOST, TCP_VPS_PORT);
    if (vps_client.connect(TCP_VPS_HOST, TCP_VPS_PORT)) {
      Serial.println("[VPS] VERBONDEN met de VPS-node");
      vps_was_connected = true;
      tcp_rx_len = 0; tcp_rx_in = false; tcp_rx_esc = false;
    } else {
      Serial.println("[VPS] verbinden MISLUKT");
    }
    return;
  }
  while (vps_client.available()) {
    uint8_t b = vps_client.read();
    if (tcp_rx_esc) {
      b ^= HDLC_ESC_MASK;
      if (tcp_rx_len < sizeof(tcp_rx_buf)) tcp_rx_buf[tcp_rx_len++] = b;
      tcp_rx_esc = false;
    } else if (b == HDLC_FLAG) {
      if (tcp_rx_in && tcp_rx_len > 0) {
        RNS::Bytes frame(tcp_rx_buf, tcp_rx_len);
        if (tcp_interface) tcp_interface.handle_incoming(frame);
      }
      tcp_rx_len = 0; tcp_rx_in = true;
    } else if (b == HDLC_ESC) {
      tcp_rx_esc = true;
    } else {
      if (tcp_rx_len < sizeof(tcp_rx_buf)) tcp_rx_buf[tcp_rx_len++] = b;
...
void setup() {
  ...
  LoRa->setPins(pin_cs, pin_reset, pin_dio, pin_busy, pin_rxen, pin_txen);
    ...
    if (LoRa->preInit()) {
      modem_installed = true;
        ...
        if (lfr == 0) {
          ...
          // Quick reboot
          #if HAS_CONSOLE
            // CBA-patch: headless standalone node — console-modus uit, die blokkeert startRadio() (regel ~984)
    ...
    if (console_active) {
      ...
      #if HAS_WIFI
        wifi_mode = WR_WIFI_STA; // CBA-patch: forceer WiFi STA voor internet-uplink naar VPS
        wifi_remote_init();
      #endif
      kiss_indicate_reset();
    }
...
#if defined(TCP_VPS)
      HEAD("Registering TCP (VPS) Interface...", RNS::LOG_TRACE);
      tcp_interface = new TCPClientInterface();
      tcp_interface.mode(RNS::Type::Interface::MODE_GATEWAY);
      RNS::Transport::register_interface(tcp_interface);
      TRACEF("TCPClientInterface hash: %s", tcp_interface.get_hash().toHex().c_str());
#endif
...
void validate_status() {
  ...
  if (boot_vector == START_FROM_BOOTLOADER || boot_vector == START_FROM_POWERON) {
    // CBA-patch: EEPROM-provisioning OPTIONEEL voor deze zelf-build.
    // Zonder lock/geldige checksum: hardcoded V4-identiteit (C8/hwrev 1) en gewoon
    // doorstarten — de oude gate hield hw_ready=false, waardoor de force-TNC-patch
    // (en dus startRadio/SX1262-init) nooit werd bereikt.
    bool eeprom_provisioned = eeprom_lock_set() && eeprom_product_valid()
                              && eeprom_model_valid() && eeprom_hwrev_valid()
                              && eeprom_checksum_valid();
    if (!eeprom_provisioned) {
      Serial.write("Device unprovisioned — CBA-patch: hardcoded V4-identiteit (model C8, hwrev 1)\r\n");
      model = MODEL_C8;
      hwrev = 1;
    }
    eeprom_ok = true;
    ...
    if (hw_ready && eeprom_provisioned && eeprom_have_conf()) {
      eeprom_conf_load();
      op_mode = MODE_TNC;
      startRadio();
    }
    // CBA-patch: dedicated standalone node — forceer TNC + hardcode thuis-radioconfig
    // (omzeilt de eeprom_have_conf/CONF_OK-persistentie deadlock)
    if (hw_ready) {
      lora_freq = 869463000;
      lora_bw   = 125000;
      lora_sf   = 8;
      ...
      op_mode   = MODE_TNC;
      startRadio();
      Serial.write(radio_online ? "CBA: startRadio() OK — radio ONLINE\r\n"
                                : "CBA: startRadio() FAALDE — zie ERROR_INITRADIO hierboven\r\n");
    }
  } else {
    hw_ready = false;
