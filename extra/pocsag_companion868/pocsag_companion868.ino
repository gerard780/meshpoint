/*
  ================================================================
  RF POWER / DUTY-CYCLE NOTICE -- READ BEFORE USING THE TX (SEND) FEATURES
  ================================================================
  This is the 868 MHz fork of extra/pocsag_companion/pocsag_companion.ino
  -- identical POCSAG codec/protocol logic, retuned from DAPNET's real
  439.9875 MHz amateur frequency to POCSAG_FREQ_MHZ (869.5875 MHz,
  see that #define below) instead. That single change moves this OUT
  of amateur radio spectrum entirely: 869.5875 MHz sits inside the
  EU868 ISM/SRD860 band's high-power sub-band (869.400-869.650 MHz,
  ETSI EN 300 220 sub-band "P"/G3), which allows up to 500 mW EIRP at
  <=10% duty cycle with NO personal amateur radio license required --
  unlike the original pocsag_companion.ino, which transmits on real
  DAPNET spectrum and does require one (see that file's own LICENSING
  WARNING, which this section replaces for this fork).

  ISM/SRD still has real regulatory limits even without a license:
  stay under the sub-band's power ceiling and duty-cycle budget for
  your jurisdiction -- this file makes no attempt to enforce duty
  cycle itself, that's on whoever's sending. The callsign gate below
  (refuses to transmit until one's configured) is kept as an optional
  courtesy/identification mechanism here, not a legal requirement the
  way it is on DAPNET's amateur spectrum.

  Also NOT the same channel as Meshpoint's own concentrator "pager"
  FSK channel (radio.pager_frequency_mhz, 869.4625 MHz by default) --
  that channel speaks a different, custom JSON-over-FSK protocol
  (pager_client.ino / pager_event_adapter.py), not real POCSAG at all.
  This file's whole point is to reach an actual POCSAG-capable pager
  (e.g. a GP2009N) directly, independent of that pipeline.
  ================================================================

  Heltec WiFi LoRa32 V3/V4 (ESP32-S3 + SX1262) -- bidirectional POCSAG
  companion, 868 MHz fork of extra/pocsag_companion/pocsag_companion.ino
  (see that file's own header for the full original design writeup --
  only what differs for this fork is called out here): listens
  continuously for POCSAG traffic on this deployment's own private
  channel and prints one JSON line per decoded page over Serial, while
  also accepting JSON lines over Serial to transmit a page live. Aimed
  at eventually being driven by meshpoint itself over USB serial, the
  same shape as the existing Meshtastic/MeshCore USB companion sources
  in src/audio/pager_listener.py's siblings (capture.meshtastic_usb /
  ...
  pocsag-companion-868.local without needing to know its DHCP address --
  ...
  http://pocsag-companion-868.local/ (styled after meshpoint's own dashboard
  ...
  Both TTGO (SX1276) and Heltec WiFi LoRa32 (SX1262) do RX+TX --
  LIVE-VERIFIED on real hardware for both (Heltec confirmed on a V3
  unit; V4 shares identical pins/radio per ttn-heltec's own port, not
  separately re-verified on this file specifically), including real
  DAPNET traffic decoded on the original 439.9875 MHz build and a full
  TX-with-callsign-prefix round trip cross-checked against the
  independent RTL-SDR/multimon-ng dashboard chain (see project memory
  for the exact confirmation log). RX bypasses PagerClient's own
  (broken-on-SX126x) direct-mode RX entirely on both chips -- it talks
  to the chip's standard FSK packet engine directly
  ...
  Heltec WiFi LoRa32 V3 or V4 (HTIT-WB32LA(F), ESP32-S3 + SX1262 --
  V3 and V4 share identical pin mappings, confirmed when ttn-heltec.ino
  was merged into one V3+V4 sketch, so this same pin block covers both;
  select the matching board in the IDE/--fqbn for flash size, nothing
  else here changes):
  ...
  RX+TX LIVE-CONFIRMED working on real V3 hardware 2026-07-29; the OLED
  pin fixes above have NOT been re-flashed/confirmed yet as of this
  comment, and V4 specifically has not been separately flash-tested
  (pins are identical to V3 per ttn-heltec's own confirmation, but this
  file's own POCSAG RX/TX has only actually been run on a V3 unit).

  Network (once WiFi connects, see secrets.h): pocsag-companion-868.local
*/

// ==== BOARD SELECT ====
// Uncomment exactly one of these before compiling. BOARD_HELTEC_WIFI_LORA32
// covers both V3 and V4 (identical pins) -- select the actual board
// (V3 vs V4) in the Arduino IDE/--fqbn for correct flash size; nothing
// in this file's own logic differs between them.
//#define BOARD_TTGO_LORA32
#define BOARD_HELTEC_WIFI_LORA32

#if defined(BOARD_TTGO_LORA32) && defined(BOARD_HELTEC_WIFI_LORA32)
  #error "Uncomment only ONE board in the BOARD SELECT block above"
#elif !defined(BOARD_TTGO_LORA32) && !defined(BOARD_HELTEC_WIFI_LORA32)
...
#elif defined(BOARD_HELTEC_WIFI_LORA32)
...
//
// MESHPOINT PAGER 869.4625 MHz
// TEST PAGER 869.5875 MHz
//
// ---------- Tuning knobs ----------
#define POCSAG_FREQ_MHZ 869.4625f // This deployment's own private ISM-band POCSAG
                                   // channel -- inside the EU868 ISM/SRD860 high-power
                                   // sub-band (869.400-869.650 MHz, 500mW/<=10% duty
                                   // cycle, no amateur license needed, see this file's
                                   // own header). 62.5kHz clear of both the mesh
                                   // channel (869.525) and the concentrator's own
                                   // non-POCSAG pager channel (869.4625) on either
                                   // side, same spacing pattern as those two already
                                   // use between each other, with margin left below
                                   // the sub-band's own 869.650 MHz upper edge.
...
// One-flag test switch: set to false and reflash to skip WiFi/NTP/OTA/web
// dashboard entirely, regardless of what's in secrets.h -- RX/TX/OLED/
// Serial all work identically either way. Exists specifically to test
// whether WiFi activity is contributing to multi-batch TX timing jitter
// (PagerClient::write()'s bit-bang loop is a tight, non-yielding busy-wait
// for the whole transmission -- long enough for a multi-batch message that
// WiFi/interrupt activity firing mid-transmission is a real candidate for
// where the bit timing drift enough to break decode past the first
// batch's worth of characters). Flip back to true for normal operation.
#define WIFI_ENABLED true

// Physical-button test send -- with WIFI_ENABLED false there's no web
// dashboard to trigger a send from, so holding the button while the
// screen is already ON fires one test send to TEST_SEND_CAPCODE with
// TEST_SEND_TEXT (see checkButton()), same effect as typing
// {"capcode": <TEST_SEND_CAPCODE>, "text": "<TEST_SEND_TEXT>"} over
// serial. A press while the screen is OFF only wakes it -- same
// "don't act on the press that was really just meant to wake the
// screen" rule already used elsewhere in this fork family
// (ttn-heltec.ino/rfenv_continuous.ino) -- only once it's already on
// does holding it do anything.
#define MY_CAPCODE 2041152UL // used when an incoming send JSON line omits "capcode"
#define TEST_SEND_CAPCODE 2041153UL
#define TEST_SEND_TEXT "button test"
#define BUTTON_LONG_PRESS_MS 700UL

#define MDNS_HOSTNAME "pocsag-companion-8682" // reachable at pocsag-companion-868.local
                                   // once WiFi connects; also used verbatim as the
                                   // ArduinoOTA hostname below so both point at the
                                   // same device. "-868" suffix (unlike the original
                                   // pocsag_companion.ino's plain "pocsag-companion")
                                   // so the two can coexist on the same network without
                                   // colliding on the same mDNS name -- this is a
                                   // second, independent device, not a replacement.
...
// (RX is TTGO/SX1276-only, never attempted on the Heltec/SX1262 build).
  ...
  #define WIFI_LED_PIN 35 // onboard LED (LED_BUILTIN in the board file, both V3+V4) --
                           // blinked by updateWifiLed() while WiFi is connected, see loop()
...
void bootLog(const String &line);
void wifiLedTick();
...
void setupWifiNtpOta() {
  ...
  while (WiFi.status() != WL_CONNECTED && millis() - start < WIFI_CONNECT_TIMEOUT_MS) {
    delay(250);
    wifiLedTick(); // loop()/updateWifiLed() isn't running yet during setup() --
                    // this blocking wait needs to blink the LED itself
    Serial.print(".");
  }
  Serial.println();

  if (WiFi.status() != WL_CONNECTED) {
    bootLog("wifi: timed out");
    return;
  }

  wifiConnected = true;
#if defined(BOARD_HELTEC_WIFI_LORA32) && WIFI_ENABLED
  digitalWrite(WIFI_LED_PIN, HIGH); // steady on the moment we're connected, rather
                                     // than waiting for loop()'s updateWifiLed() to
                                     // catch up once setup() finally returns
#endif
  bootLog("wifi: " + WiFi.localIP().toString());

  if (MDNS.begin(MDNS_HOSTNAME)) {
    bootLog("mdns: " + String(MDNS_HOSTNAME) + ".local");
  } else {
    bootLog("mdns: begin() failed");
  }
  ...
  Serial.println();
  bootLog(getLocalTime(&timeinfo, 0) ? "ntp: synced" : "ntp: timed out");
...
// Time-based debounce (50ms) -- GPIO0 is active-low with an internal
// pull-up, so idle reads HIGH and a press reads LOW. Three gestures:
//   - Press while the screen is OFF: wakes it only, see
//     TEST_SEND_CAPCODE's own comment for why this press doesn't also
//     count as either of the two gestures below.
//   - Short press while the screen is ON: toggles it back off (the
//     only gesture this used to have).
//   - Long press (>= BUTTON_LONG_PRESS_MS) while the screen is ON:
//     fires one test send via sendPocsagAlpha(TEST_SEND_CAPCODE,
//     TEST_SEND_TEXT) -- see that define's own comment.
// Blinks while WiFi is disconnected (boot/connecting/dropped -- one signal
// throughout), steady on once connected. If WIFI_ENABLED is false, WiFi is
// never even attempted, so the LED just stays off (set once in setup()) --
// a permanently-blinking LED on an intentionally RX/TX-only unit would be
// misleading. This handles the steady-state (post-setup, called every
// loop() tick); the blocking WiFi-connect wait inside setupWifiNtpOta()
// (and the boot delay in setup(), before loop() ever runs) toggle the LED
// themselves at the same points they already block, via wifiLedTick().
...
void updateWifiLed() {
  ...
  if (wifiConnected) {
    if (!ledOn) { ledOn = true; digitalWrite(WIFI_LED_PIN, HIGH); }
    return;
  }
  if (millis() - lastToggle >= WIFI_LED_BLINK_MS) {
    lastToggle = millis();
    ledOn = !ledOn;
    digitalWrite(WIFI_LED_PIN, ledOn ? HIGH : LOW);
  }
#endif
}

// One manual toggle, for use inside setup()'s blocking delay()/wait loops
// (before loop()/updateWifiLed() ever gets a chance to run) -- boot delay
// and the WiFi-connect wait, so the LED is already blinking from power-on,
// not just once setupWifiNtpOta() gets far enough to start polling status.
void wifiLedTick() {
#if defined(BOARD_HELTEC_WIFI_LORA32) && WIFI_ENABLED
  static bool ledOn = false;
  ledOn = !ledOn;
...
void checkButton() {
  ...
  static bool gestureFired = false; // true once THIS press has already done its one thing (wake, short, or long)
  ...
  if (millis() - lastChangeAt > 50) {
    if (reading != debouncedState) {
      ...
      if (debouncedState == LOW) {
        ...
        gestureFired = false;
        if (!displayOn) {
          toggleDisplay(); // wake only
          gestureFired = true; // suppress the short/long branches below for the rest of this press
        }
      } else if (!gestureFired) {
        // just released, and neither the wake case nor the long-press
        // case (below) already claimed this press -- a short press.
        toggleDisplay();
      }
    }
    if (debouncedState == LOW && !gestureFired && millis() - pressStartAt >= BUTTON_LONG_PRESS_MS) {
      gestureFired = true;
      sendPocsagAlpha(TEST_SEND_CAPCODE, TEST_SEND_TEXT);
    }
  }
}
...
// ---------- RX decode ----------
//
// decodeBcdChar()/decodeBatchAndEmit() are a verbatim port of the same
// logic already verified (compiled and reasoned through against
// RadioLib's own readData() source, see the comments inline) in
// pocsag_test.ino's RX-mode branch. decodeMessageSymbolsInto() started
// as that same port (as decodeMessageSymbols()) but has since grown
// cross-batch continuation support (PendingPage, below) that
// pocsag_test.ino's own version doesn't have -- see that function's
// own comment for why.
...
// Carries decode state (overflow/ovfBits/prevCw, the mid-character
// split-across-a-codeword-boundary bits, plus the accumulated text so
// far) IN and OUT via pointers/reference instead of always starting
// fresh, so a caller can resume exactly where a previous call left off
// -- across a batch boundary, not just within one array. Returns true
// if decode reached a natural stop (IDLE, or another address codeword)
// within codewords[0..n); false if it ran out of array first (idx hit
// n) with the page potentially still continuing into the next batch --
// *outConsumed reports how many codewords were used either way, so the
// caller can pick up its own scan right after them.
//
// FRAME_SYNC gets skipped (continue), not treated as a stop -- matches
// simDecode()'s own handling (see that function, used by
// padAlphaForCleanDecode() to self-check every outgoing message before
// it's ever sent). The two decoders had diverged here before: this one
// used to break on FRAME_SYNC same as IDLE, which both (a) could never
// recover mid-batch if bit corruption produced a false-positive match
// for the sync pattern, where continuing might still salvage the rest
// of the message, and (b) silently truncated any genuinely multi-batch
// message at the first batch boundary even with error-free reception --
// LIVE-CONFIRMED as a real, reproducible bug (not RF noise): capcodes
// whose frame position sits late in a batch (framePos = capcode & 0x07,
// e.g. framePos 7 leaves exactly one 20-bit codeword after the address,
// room for exactly 2 characters) truncated identically every single
// time regardless of signal quality, because there was nowhere for
// decode to continue INTO -- decodeBatchAndEmit() only ever saw one
// batch's 16 codewords per call. See PendingPage below for the other
// half of the actual fix -- accumulating across calls, not just
// tolerating a sync word within one.
bool decodeMessageSymbolsInto(uint32_t *codewords, size_t n, size_t startIdx, uint8_t symbolLength, bool bcd,
                               String &out, bool &overflow, int8_t &ovfBits, uint32_t &prevCw, size_t *outConsumed) {
  size_t idx = startIdx;

  for (; idx < n; idx++) {
    uint32_t cw = codewords[idx];
    if (cw == RADIOLIB_PAGER_IDLE_CODE_WORD) { *outConsumed = idx - startIdx; return true; }
    if (cw == RADIOLIB_PAGER_FRAME_SYNC_CODE_WORD) continue;
    bool isMessage = (cw & (RADIOLIB_PAGER_MESSAGE_CODE_WORD << (RADIOLIB_PAGER_CODE_WORD_LEN - 1))) != 0;
    if (!isMessage) { *outConsumed = idx - startIdx; return true; }
  ...
  *outConsumed = idx - startIdx;
  return false; // ran out of this batch's codewords, possibly still open
}

// A page whose decode ran out of the current batch's codewords before
// reaching a natural stop (see decodeMessageSymbolsInto()'s own
// comment) -- held here so the very start of the NEXT received batch
// resumes it instead of decodeBatchAndEmit() silently emitting a
// truncated page. Only one at a time: decode is strictly sequential
// through a batch's codewords, so at most one page can still be "open"
...
// How long a page is allowed to stay "open" waiting for its
// continuation before it's abandoned instead of resumed. One POCSAG
// batch at 1200 baud takes ~450ms to transmit (32-bit sync word + 16
// 32-bit codewords), so consecutive batches of one real, continuous
// transmission arrive well under this -- a gap this long means the
// transmission actually ended (or was lost), and resuming into
// whatever arrives next would silently stitch two unrelated pages
// together instead of just truncating one, a worse failure than the
// bug this replaces.
#define PENDING_PAGE_MAX_AGE_MS 2000UL
...
// Builds the type label, applies the numeric-housekeeping filter, and
// emits one fully-decoded page as JSON/Serial/web-log/OLED. Shared by a
// page that finishes within the same batch it started in and one
// resumed from PendingPage that only finishes in a later batch -- both
// go through this exact same path, so there's only one place that
// defines what "a decoded page" actually does.
void emitDecodedPage(uint32_t capcode, uint8_t function, String text) {
...
// Walks one full 16-code-word batch and emits one decoded page (via
// emitDecodedPage()) per page found in it (a batch can carry pages for
// up to 8 different capcodes at once, one per frame position). See
// pocsag_test.ino's RX-mode comment for how the capcode/framePos
// reconstruction was confirmed against PagerClient::readData().
//
// Resumes PendingPage FIRST, before scanning this batch's own
// addresses -- continuation codewords for a page still open from the
// previous batch come right at the start of this one, and nothing new
// can begin until whatever was already in progress actually ends (see
// PendingPage's own comment for why this exists at all: a page whose
// frame position leaves little or no room before the batch boundary --
// or one that's just genuinely long -- has nowhere to continue INTO
// without this).
int rxBatchCount = 0;
...
void decodeBatchAndEmit(uint32_t *cw, size_t n) {
  ...
  if (pendingPage.active) {
    if (millis() - pendingPage.startedAtMs > PENDING_PAGE_MAX_AGE_MS) {
      // Too long since the batch that opened this page for it to still
      // be the same transmission -- emit whatever was decoded so far
      // (same as the old single-batch behavior) rather than risk
      // stitching this new, unrelated batch onto it.
      emitDecodedPage(pendingPage.capcode, pendingPage.function, pendingPage.text);
      pendingPage.active = false;
    } else {
      size_t consumed = 0;
      bool finished = decodeMessageSymbolsInto(cw, n, 0, pendingPage.bcd ? 4 : 7, pendingPage.bcd,
                                                pendingPage.text, pendingPage.overflow, pendingPage.ovfBits,
                                                pendingPage.prevCw, &consumed);
      i = consumed;
      if (finished) {
        emitDecodedPage(pendingPage.capcode, pendingPage.function, pendingPage.text);
        pendingPage.active = false;
      }
      // else: still open, stays pending into yet another batch -- i is
      // already == n here (this whole batch was consumed as
      // continuation too), so the scan below naturally finds nothing
      // more this round.
    }
  }
  ...
  for (; i < n; i++) {
    ...
    uint32_t prevCw = 0;
    size_t consumed = 0;
    bool finished = decodeMessageSymbolsInto(cw, n, i + 1, symbolLength, bcd, text, overflow, ovfBits, prevCw, &consumed);

    if (finished) {
      emitDecodedPage(capcode, function, text);
    } else {
      // Ran out of this batch's codewords before the page naturally
      // ended -- hold it for the next batch (see PendingPage's own
      // comment) instead of emitting it silently truncated right now.
      pendingPage.active = true;
      pendingPage.capcode = capcode;
      pendingPage.function = function;
      pendingPage.bcd = bcd;
      pendingPage.text = text;
      pendingPage.overflow = overflow;
      pendingPage.ovfBits = ovfBits;
      pendingPage.prevCw = prevCw;
      pendingPage.startedAtMs = millis();
    }
...
// Flushes a PendingPage that's been waiting too long for its
// continuation, independent of whether another packet ever arrives.
// decodeBatchAndEmit()'s own staleness check (see PENDING_PAGE_MAX_AGE_MS)
// only runs when a NEW packet shows up -- without this, a page whose
// continuation never arrives at all (RF glitch, the second batch's
// sync word landing in the software re-arm gap between
// radio.readData() and the next startReceive() -- see loop()'s own RX
// comment, or the transmission genuinely only had one batch's worth)
// would sit silently pending FOREVER instead of eventually showing
// what was actually decoded, worse than the old single-batch
// behavior's immediate (if truncated) result. Called every loop() tick
// from the RX section below -- cheap (one bool check) when nothing's
// pending.
void flushStalePendingPage() {
  if (!pendingPage.active) return;
  if (millis() - pendingPage.startedAtMs <= PENDING_PAGE_MAX_AGE_MS) return;
  emitDecodedPage(pendingPage.capcode, pendingPage.function, pendingPage.text);
  pendingPage.active = false;
}
...
#if defined(BOARD_TTGO_LORA32)
  radio.beginFSK(POCSAG_FREQ_MHZ, 4.8, 5.0, 125.0, 22, 16, false);
#elif defined(BOARD_HELTEC_WIFI_LORA32)
  radio.beginFSK(POCSAG_FREQ_MHZ, 4.8, 5.0, 125.0, 22, 16);
#endif
  // +22dBm is the SX1262's actual max (confirmed against RadioLib's own
  // SX1262::checkOutputPower(): RADIOLIB_MAX(-9, RADIOLIB_MIN(22, power))
  // -- not an arbitrary pick), same as the concentrator's own default
  // transmit.tx_power_dbm. Still well under the ISM sub-band's 500mW/
  // 27dBm regulatory ceiling either way -- the chip itself can't reach
  // that here, so there's no headroom being left on the table by using
  // its actual max instead of a lower figure.
...
void handleSerialJsonLine(const String &line) {
  ...
  uint32_t capcode = doc["capcode"] | MY_CAPCODE;
...
<title>POCSAG Companion (868)</title>
<style>
  :root {
    --bg-primary: #0a0e17;
    --bg-card: #162033;
...
// Scrolling boot console -- each call appends one line, prints it to Serial
// too, and redraws the whole 8-line buffer (8px/line exactly fills the
// 64px OLED). Used only during setup()/setupWifiNtpOta() startup; the
// normal RX/TX screens (oled()/showListeningScreen()/etc.) take over once
// boot finishes.
#define BOOT_LOG_MAX_LINES 8
void bootLog(const String &line) {
  static String lines[BOOT_LOG_MAX_LINES];
  static uint8_t count = 0;
  Serial.println(line);
  // 128px wide / 6px per glyph at textSize(1) = ~21 chars/row -- anything
  // longer wraps onto the next row and collides with that row's own line
  // (same 21-char limit showIncomingScreen() already uses for message text).
  String trimmed = line.length() > 21 ? line.substring(0, 21) : line;
  if (count < BOOT_LOG_MAX_LINES) {
    lines[count++] = trimmed;
  } else {
    for (uint8_t i = 1; i < BOOT_LOG_MAX_LINES; i++) lines[i - 1] = lines[i];
    lines[BOOT_LOG_MAX_LINES - 1] = trimmed;
  }
...
void showListeningScreen() {
  ...
  display.print(POCSAG_FREQ_MHZ, 4);
  display.print("MHz  ");
  ...
  // MY_CAPCODE is a compile-time #define -- if it's being
  // hand-edited differently before each unit's own flash (e.g. so a
  // multi-unit test bench's devices default to sending to each other),
  // this is the only way to tell which one a given unit was actually
  // built with after a reboot, without checking the source.
  display.setCursor(0, 50);
  display.print("My capcode: ");
  display.print(MY_CAPCODE);
  display.display();
}
...
void setup() {
  Serial.begin(115200);
#if defined(BOARD_HELTEC_WIFI_LORA32)
  pinMode(WIFI_LED_PIN, OUTPUT); // set up before the boot-delay blink below,
                                  // not later, so digitalWrite() here actually drives it
  digitalWrite(WIFI_LED_PIN, LOW);
#endif
#if WIFI_ENABLED
  for (uint8_t i = 0; i < 4; i++) { delay(250); wifiLedTick(); } // same 1s wait as
                                    // before, but blinking from power-on rather than
                                    // waiting for setupWifiNtpOta() to start it later
#else
  delay(1000);
#endif
  ...
  bootLog("prefs: loaded");

  configureForRx();
  bootLog("radio: " + String(POCSAG_FREQ_MHZ, 4) + "MHz");
  setupWifiNtpOta(); // best-effort, bounded timeout -- see its own comment
  bootLog("boot done");
  delay(600); // let the last boot-log line sit on screen briefly before showListeningScreen() replaces it

  Serial.println("========================================");
#if defined(BOARD_TTGO_LORA32)
  Serial.println("Board: TTGO LoRa32 (SX1276) -- RX+TX, LIVE-VERIFIED");
#elif defined(BOARD_HELTEC_WIFI_LORA32)
  Serial.println("Board: Heltec WiFi LoRa32 V3/V4 (SX1262) -- RX+TX, both LIVE-VERIFIED");
#endif
  Serial.printf("POCSAG companion ready -- %.4f MHz, no DIO2 needed\n", (double)POCSAG_FREQ_MHZ);
  if (wifiConnected) {
...
void loop() {
  ...
  updateWifiLed();     // onboard LED blinks while WiFi is connected, off otherwise
  ...
  flushStalePendingPage(); // every tick, not just when a new packet arrives -- see its own comment for why
  ...
  if (digitalRead(LORA_IRQ_PIN)) {
    ...
    // Re-arm BEFORE decoding/displaying, not after -- decodeBatchAndEmit()
    // below can take a real handful of milliseconds (JSON serialize,
    // Serial print, an OLED redraw over I2C), and this used to only
    // re-arm on the NEXT loop() iteration, after that AND a full lap of
    // every other per-tick check (checkSerialInput()/checkButton()/
    // ArduinoOTA.handle()/checkWebSendPending()/etc.). LIVE-CONFIRMED
    // this was the actual reason PendingPage's cross-batch continuation
    // never completed even once: the next batch of a genuinely
    // multi-batch message arrives close behind the first with barely a
    // gap, easily shorter than that whole delay -- the continuation was
    // being missed at the radio, not failing to decode once received.
    int rearmState = radio.startReceive();
    if (rearmState != RADIOLIB_ERR_NONE) {
      Serial.print("[rx] re-arm startReceive() FAILED, code="); Serial.println(rearmState);
    }
    rxContinuousStarted = true;
