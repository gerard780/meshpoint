 ...
 *
 * Shared across every place that renders a node's `hardware_model`
 * (a stringified raw enum int, e.g. "70") -- node_drawer.js's Node Info
 * panel, node_cards.js's node-list chips, and stats_tab.js's hardware
 * distribution chart. Loaded once, early, as a plain global (matches
 * this codebase's existing non-module script style) so every consumer
 * can reference `HW_NAMES` without re-declaring or importing it.
 */
const HW_NAMES = {
    0: 'UNSET',
