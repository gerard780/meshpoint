...
Same shape as ``lorawan_routes.py`` (a one-way receive protocol with
no conversations) but keyed on ``capcode`` (DAPNET's addressing unit)
rather than ``dev_eui``/``dev_addr``, and with no RF signal columns --
the companion board reports no RSSI/SNR over its JSON serial protocol.
"""
