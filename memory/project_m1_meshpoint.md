...
**Not yet flashed onto real hardware** -- next step when picked back up is an actual live flash of the repurposed Heltec V3 (or the TTGO board) via this new card, confirming the downloaded firmware actually boots as Meshtastic and the Serial capture source picks it up correctly afterward (same "does the port survive/reconnect" question already answered for the POCSAG card, not yet confirmed for this one).
