...
| Status | Effort | Item |
|--------|--------|------|
| Open -- unblocked 2026-07-31 | M | **Run `dab_channel_scan.py` from the dashboard (DAB+ Config page) with a live output view**, instead of CLI-only over SSH. User's ask 2026-07-31 while running the sid-capture-verification scan by hand -- explicitly scoped as a follow-up at the time ("note this dont do it yet thats after making this sid working"). Same long-standing gap already flagged in the 2026-07-19 DAB+ Config narrative further down this file. Concrete shape confirmed by the user: **"same as the update firmware buttons (sort of) from the companions"** -- i.e. reuse `window.UpdateStreamClient.postNdjson()`, the exact shared client helper the Meshtastic/MeshCore/POCSAG firmware-flash cards already use (`serial_card.js`'s `data-mt-firmware-flash` button is the concrete reference: disables itself + streams `{type:'started',cmd}`/`{type:'line',text}` events into a live output `<pre>` + shows a final success/error status), not a from-scratch UI. A ~19-minute worst-case full scan genuinely needs this kind of live visibility, not a fire-and-forget trigger. Backend side would need a new streaming route mirroring `.../firmware/flash/stream`'s NDJSON-over-subprocess pattern, running `dab_channel_scan.py` instead of esptool/arduino-cli. Its blocker (sid-capture fix confirmed working) cleared the same day -- live-verified against a real scan with real sids captured -- so this is now ready to pick up whenever wanted |
