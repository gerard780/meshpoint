...
| Likely fixed, pending live confirmation | S | **Favorite click sometimes doesn't start the stream on the first try** — user observed live (2026-07-20): clicking a favorite correctly triggers the tune/scan and is *supposed* to auto-play once the station decodes, but sometimes nothing actually starts playing; manually opening that channel's tab and pressing Play once "fixes" it, and favorites work fine afterward. Suspected cause (name-based `_resolvePendingPlay()`/`_favMatches()` matching silently failing when a live-decoded label drifts from the scanned name) is exactly what the sid-capture item above eliminates -- favorites/presets now carry a real, stable sid once rescanned, giving exact-identity matching instead of the fuzzy `trim().toLowerCase()` fallback. Not marked DONE outright: this was never independently reproduced/root-caused with a captured repro case (unlike the sid-capture fix itself, which was verified against the exact failure mechanism in isolation) -- it's a strong, well-evidenced diagnosis, not a confirmed-then-fixed bug. Needs a real rescan on the Pi + some live favorite clicks to confirm the reported symptom is actually gone, not just that its suspected cause was removed |
