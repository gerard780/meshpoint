...
**Configuration → Firmware** has a Compile/Flash card for this board (`src/api/routes/rfenv_companion_firmware_routes.py`, `rfenv_companion_firmware_card.js`) — same `arduino-cli` streaming mechanism as the Pager/POCSAG cards, single board (no picker, matching the Pager card's own simplicity), device-to-flash picker plus Compile/Flash/Show-output. If the selected port is the currently-configured companion, the live `RfEnvCompanionScanService` is released before flashing and reconnected afterward (same pattern as the POCSAG card's own release/reconnect logic). No compile-time fields yet (no capcode-style injection) — a per-device frequency override may be added here later.
