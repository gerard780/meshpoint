...
Flash `extra/rfenv_companion/rfenv_companion.ino` (RadioLib 7.7.1, same board/pins already proven by `extra/pager_client`) — no WiFi/secrets needed, it's polled directly over the USB serial connection. The board's own OLED has three screens, all working independent of any USB/Meshpoint connection: a **status** screen (last poll time), a **live band-scan** view (continuous RSSI sweep across the board's own EU868-default band constant, drawn in real time), and a **local scan** screen (hold the PRG button — runs the exact same single-frequency histogram scan Meshpoint itself requests, with floor/median shown right on-device). Short-press cycles status/live-scan; the hold gesture works from either.
