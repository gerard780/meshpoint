...
- **Concentrator FSK support (internal, not yet reachable from the dashboard).** The SX1302's dedicated FSK IF chain (ch9) — genuinely independent hardware from the LoRa channels, confirmed against the patched HAL's own source — can now be configured for RX, laying the groundwork for the planned emergency pager feature. No user-visible effect yet; nothing wires this up to a real frequency or capture source.
