"""REGION_PRESETS (MeshCore community radio presets) and region resolution.

REGION_PRESETS used to be a hand-picked 3-entry table (US/EU/ANZ). It now
carries the full 20-entry list transcribed from the official MeshCore
client's own "Select Radio Settings" picker, so these values are pinned
here to catch any future transcription slip -- not just exercising the
resolution logic around them.
"""
...
class RegionPresetsTest(unittest.TestCase):
    ...
    def test_eu_433_pair_present(self):
        ...
        self.assertEqual(
            (long_range.frequency_mhz, long_range.bandwidth_khz,
             long_range.spreading_factor, long_range.coding_rate),
            (433.650, 250.0, 11, 5),
        )
        self.assertEqual(
            (narrow.frequency_mhz, narrow.bandwidth_khz,
             narrow.spreading_factor, narrow.coding_rate),
            (433.650, 62.5, 8, 8),
        )
...
class ResolveRegionTest(unittest.TestCase):
    def test_legacy_short_aliases_map_to_the_original_us_eu_anz_values(self):
        # These three used to BE the entire REGION_PRESETS table under
        # these exact keys -- confirms the full-list migration didn't
        # silently change the values anyone relying on them would get.
        ...
        self.assertEqual(REGION_PRESETS["EU_UK_NARROW"].coding_rate, 8)
        self.assertEqual(REGION_PRESETS["AUSTRALIA_NARROW"].frequency_mhz, 916.575)

    def test_eu_868_alias_matches_meshpoint_own_region_naming(self):
        self.assertEqual(_resolve_region("EU_868"), "EU_UK_NARROW")

    def test_full_preset_key_resolves_case_insensitively(self):
        self.assertEqual(_resolve_region("eu_433_narrow"), "EU_433_NARROW")

    def test_custom_resolves_lowercase(self):
    ...
    def test_preset_choices_includes_every_key_plus_custom(self):
        self.assertEqual(set(PRESET_CHOICES), set(REGION_PRESETS.keys()) | {"custom"})
